        ...
        arrCustomer = [
            {
                user: 'admin',
                password: 'admin',
                email: 'nguyenthilinh1942002@gmail.com',
                level: 0,
                comment:[],
                status:"Hoạt động"
            },
...
{
    document.getEle.mentById('barsCategoryItem').style.display = "none";
...
function searchInfo()
{
    ...
    const EmptyProductFilter = Info.filter(value => {
        return value.title.toUpperCase().includes(valuesearch.toUpperCase())
    })
    renderEmptyProduct(EmptyProductFilter);
}
function renderEmptyProduct(arr)
{
    let post = '';
    arr.map(value => {
        post += '<div class = "item">${value.id}. ${value.title}<div>';
    })
